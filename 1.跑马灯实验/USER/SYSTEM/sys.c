 ...
 */
static uint8_t sys_clock_set(uint32_t plln)
{
...
void sys_stm32_clock_init(uint32_t plln)
{
    ...
    RCC->AHBENR = 0x00000000;       /* AHB1外设时钟关闭 */
    RCC->APB2ENR = 0x00000000;      /* AHB2外设时钟关闭 */
 ...
 * WFI is an instruction used in the ARM architecture (and other architectures as well). 
When a processor executes a WFI instruction, it enters a low - power state and waits for an interrupt to occur. 
 * @param       无
 * @retval      无
 */
