...
void lcd_show_char(uint16_t x, uint16_t y, char chr, uint8_t size, uint8_t mode, uint16_t color)
{
    ...
    fontIndex = (uint8_t)(chr - ' ');    /* 得到偏移后的值（ASCII字库是从空格开始取模，所以-' '就是对应字符的字库） */

    switch (size)
    {
        case 12:
            pfont = (uint8_t *)asc2_1206[fontIndex];  /* 调用1206字体 */
            break;

        case 16:
            pfont = (uint8_t *)asc2_1608[fontIndex];  /* 调用1608字体 */
            break;

        case 24:
            pfont = (uint8_t *)asc2_2412[fontIndex];  /* 调用2412字体 */
            break;

        case 32:
            pfont = (uint8_t *)asc2_3216[fontIndex];  /* 调用3216字体 */
            break;
